...
class Model:
    ...
    def model_train(self, experiences):
        ...
        for i in range(1000):
            value_loss, state_loss, _, _ = self.sess.run([self.value_loss, self.value_run_train, self.state_loss, self.state_run_train], feed_dict=feed_dict)
        return value_loss
    ...
    def dqn_train(self, experiences):
        ...
        for experience in training_experiences:
            ...
            for i in range(2):
                ...
                state0Reshaped = np.reshape(state0, (1, self.state_size))
                if not state0Reshaped in X:
                    X = np.concatenate((X, state0Reshaped), axis=0)
                    Y = np.concatenate((Y, np.reshape(actions0, (1, self.action_size))), axis=0)
