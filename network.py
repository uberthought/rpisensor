...
class Model:
    def __init__(self, state_size, action_size):
        ...
        units = 32
        # value_hidden0 = tf.layers.dense(inputs=self.states0, units=units, activation=tf.nn.relu)
        # self.value_prediction = tf.layers.dense(inputs=value_hidden0, units=1)
        ...
        model_hidden0 = tf.concat([self.states0, self.actions], axis=1)
        ...
        # model_hidden3 = tf.layers.dense(inputs=model_hidden2, units=units, activation=tf.nn.relu)
        ...
        dqn_hidden1 = tf.layers.dense(inputs=self.states0, units=units, activation=tf.nn.relu)
        dqn_hidden2 = tf.layers.dense(inputs=dqn_hidden1, units=units, activation=tf.nn.relu)
        dqn_hidden3 = tf.layers.dense(inputs=dqn_hidden2, units=units, activation=tf.nn.relu)
